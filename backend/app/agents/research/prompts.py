...
6. 长度建议 1200–2200 字。
7. 若证据适合可视化（时间线、人物关系、阶段对比、数量统计等），在「核心发现」相关节落后追加 **1–3 个** 结构化附图围栏（优先），格式如下：
```artifact-viz
{{
  "title": "简短中文标题",
  "kind": "echarts",
  "option": {{ /* 合法 ECharts option；仅用证据中的真实数据，禁止伪造 */ }}
}}
```
   - `kind` 还可为 `timeline`（需 `items: [{{"time":"...","name":"...","detail":"..."}}]`）或 `network`（需 `nodes`/`links`）。
   - 每个围栏必须是完整合法 JSON；无合适可视化时不要输出。
   - 仅当结构化规格难以表达时，才可用次选 ```artifact-html（完整可运行 HTML，可 CDN 引入 ECharts；勿访问 cookie/localStorage）；合计附图不超过 3 个。
